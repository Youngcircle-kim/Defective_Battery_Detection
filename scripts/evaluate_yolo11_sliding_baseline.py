"""Evaluate YOLOv11 sliding-crop baseline on full images."""
...
from infer_yolo11_sliding_baseline import predict_sliding
...
def save_debug_panel(
    ...
    panel = np.concatenate([img, gt_vis, pred_vis], axis=1)
    ...
    cv2.imwrite(str(vis_dir / f"{stem}_compare.png"), panel)
...
def gt_masks_from_label(
    ...
    for cls_name in ["battery_outline", "damaged", "pollution"]:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="runs/sliding_eval")
    ...
    if args.image_dir is not None:
        ...
        if args.label_dir is None:
            raise ValueError("--image-dir 사용 시 --label-dir 필요")
    ...
    print("[Sliding Evaluation Config]")
    ...
    for img_path in tqdm(img_paths, desc="evaluate sliding"):
